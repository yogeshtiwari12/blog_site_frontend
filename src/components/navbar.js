    ...
    <div className="relative w-full bg-white shadow-lg">
      <div className="mx-auto flex max-w-8xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:flex space-x-4">
          {cookie && profile?.userprofile?.role === 'admin' && (
            <Link
              to="/dashboard"
          ...
          <ul className="space-y-2">
            ...
            <li>
              {cookie ? (
                <button
                  onClick={logout}
                  className="block w-full rounded-md bg-red-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-400 text-center"
                >
                  Logout
                </button>
              ) : (
